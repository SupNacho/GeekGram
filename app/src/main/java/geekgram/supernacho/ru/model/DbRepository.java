...
public class DbRepository {
    ...
    public void remove(PhotoModel pm) {
        ...
        if (removed) {
            Realm realm = Realm.getDefaultInstance();
            RealmResults<RealmImage> realmPhotos = realm.where(RealmImage.class)
                    .equalTo("imgUri", pm.getPhotoSrc()).findAll();
            realm.executeTransaction(innerRealm -> realmPhotos.deleteAllFromRealm());
            dbRepoEventBus.onNext(RepoEvents.DB_UPDATED);
            realm.close();
            disposeDeleteProcessor = Observable.just(new File(pm.getPhotoSrc()
                    .substring(5, pm.getPhotoSrc().length())))
                    .subscribeOn(Schedulers.io())
                    .delay(5, TimeUnit.SECONDS)
                    .doOnDispose(() -> Timber.d("Deletion disposed"))
